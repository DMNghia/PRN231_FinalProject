...
namespace FinalProject.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public AuthController(JwtService jwtService, ILogger<AuthController> logger, Prn231_FinalProjectContext context, EmailService emailService)
        {
        ...
        public async Task<IActionResult> ActiveAccount(string tokenValue)
        {
            ...
            Token? token = await _context.Tokens.FirstOrDefaultAsync(t => t.TokenValue == tokenValue);
            if (token == null)
            {
                return BadRequest(new BaseResponse<object>
                {
                    code = ResponseCode.ERROR.GetHashCode(),
                    message = "Token không chính xác vui lòng vào mail của bạn để kích hoạt hoặc đăng nhập để nhận lại mail xác nhận mới"
                });
            }
            if (token.IsUse == true)
            {
                ...
                {
                    code = ResponseCode.ERROR.GetHashCode(),
                    message = "Token đã được sử dụng vui lòng vào mail của bạn để kích hoạt hoặc đăng nhập để nhận lại mail xác nhận mới"
                });
            }
            if (token.ExpiredTime < DateTime.UtcNow)
            {
                ...
                {
                    code = ResponseCode.ERROR.GetHashCode(),
                    message = "Token đã hết hạn vui lòng vào mail của bạn để kích hoạt hoặc đăng nhập để nhận lại mail xác nhận mới"
                });
            }
            User? user = await _context.Users.FirstOrDefaultAsync(u => u.Id == token.UserId);
            if (user == null)
            {
                logger.LogError($"ACTIVE ACCOUNT TOKEN VALUE: {tokenValue} ERROR >>> USER NOT FOUND");
                return BadRequest(new BaseResponse<object>
                {
                    code = ResponseCode.ERROR.GetHashCode(),
        ...
        {
            logger.LogInformation($"START SIGN IN WITH EMAIL: {request.Email}");
        ...
        public async Task<IActionResult> SignUp([FromBody] SignUpRequest request)
        {
            try
            {
                User? user = await _context.Users.FirstOrDefaultAsync(u => u.Email == request.Email && u.TypeAuthentication == TypeAuthentication.LOCAL.ToString());
                if (user != null)
                {
                    logger.LogInformation($"SIGN UP FOR EMAIL: {request.Email} >>> ERROR EMAIL ALREADY EXIST");
                    return BadRequest(new BaseResponse<object>
                    {
                        code = ResponseCode.ERROR.GetHashCode(),
                ...
                string hashPassword = BCrypt.Net.BCrypt.EnhancedHashPassword(request.Password);
                User newUser = new User
                {
                    ...
                    IsActive = false,
                    Role = request.Role,
                    TypeAuthentication = TypeAuthentication.LOCAL.ToString(),
                };
                _context.Users.Add(newUser);
                await _context.SaveChangesAsync();
                logger.LogInformation($"SAVE NEW USER SUCCESS START SEND EMAIL TO {request.Email}");
                SendActiveAccount(request.FullName, request.Email, newUser.Id);
                return Ok(new BaseResponse<object>
                {
                    code = ResponseCode.SUCCESS.GetHashCode(),
        ...
        private void SendActiveAccount(string name, string email, int userId)
        {
            string activeToken = Guid.NewGuid().ToString() + DateTime.Now.Second;
